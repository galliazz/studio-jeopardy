...
- Public OBS mirror overlays
  - [x] `games.overlay_token` + public token-scoped read + regenerate
  - [x] `/overlay/board/$token`, `/overlay/queue/$token`, `/overlay/combined/$token`
        (1920x1080 fixed canvas, transparent, realtime only, no polling)
  - [x] Host "OBS overlay links" panel: real URLs, copy + open, regenerate behind confirm,
