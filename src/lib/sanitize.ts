 ...
 *
 * Due riscritture successive:
 *
 *  1. La prima versione era una LISTA NERA di espressioni regolari, aggirabile
 *     in modo banale (`<img src=x/onerror=…>` passava perché il filtro cercava
 *     uno spazio prima dell'attributo).
 *
 *  2. La seconda passava a lista bianca ma FALLIVA APERTO: quando la regex non
 *     riconosceva un tag — per esempio per una virgoletta spaiata — il testo
 *     veniva ricopiato tale e quale nell'output, tag compreso. E scartando un
 *     tag ignoto si poteva ricomporre markup vivo: `<<x>img src=…>` diventava
 *     `<img src=…>`.
 *
 * Questa versione FALLISCE CHIUSO: ogni `<` che non apra un tag riconosciuto ed
 * esplicitamente permesso viene trasformato in `&lt;`. Ne segue anche che il
 * risultato è stabile — sanificare due volte dà lo stesso testo — perché dopo
 * la prima passata non restano `<` ambigui.
 *
 ...
 * lista nera di pattern pericolosi si aggira con commenti (`ur/**\/l(...)`),
 * entità HTML o escape CSS, che qui semplicemente non superano la validazione.
 */
